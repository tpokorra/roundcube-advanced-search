(function($) {
    ...
    rcmail.addEventListener('plugin.load_search', function(r) {
        ...
        $('tr.criteria', $('tbody', $form)).not(':first').remove();
        ...
        $.each($obj.search, function($key, $value) {
            if (!$first) {
                $('tr.criteria', $('tbody', $form)).after($.stack.row);
            }
            $first = false;
            $tr = $('tr.criteria', $('tbody', $form));
            $tr = $tr[$tr.length - 1];

            $row = $value;
            $('select[name=method]', $tr).val($row.method);
    ...
    $('input[name=save]').live('click', function(e) {
        ...
        if ($tr.length) {
            $tr.each(function() {
                // data.push( {test: $('input[name=not]', $(this)).attr('checked')});
                var item = {not: $('input[name=not]', $(this)).attr('checked') == 'checked',
    ...
    /**
     * The changed selection event handler for the saved searches list box. This loads the selected search.
     *
     * @param {object} e The event element
     */
